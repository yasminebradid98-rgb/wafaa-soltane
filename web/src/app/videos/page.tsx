...
export default async function VideosPage() {
  const [projects, videos] = await Promise.all([
    ...
    `),
    client.fetch(`*[_type == "video"]{ _id, title, videoUrl }`),
  ])
  ...
  return (
    <div className="dark:bg-black bg-white min-h-screen text-black dark:text-white px-5 md:px-20">
      ...
      <div className="container mx-auto py-10">
        <h1 className="text-4xl font-bold uppercase mb-2">Videography</h1>
        <p className="text-lg text-gray-500 dark:text-gray-400 font-serif mb-8">
          Video Works & Visual Experiments
        </p>
          ...
          {videos && videos.length > 0 ? (
            videos.map((vid: any) => (
              <div key={vid._id} className="space-y-2">
                <div className="overflow-hidden rounded-sm bg-black aspect-video">
                  <video controls className="w-full h-full object-cover">
                    <source src={vid.videoUrl} />
                    Your browser does not support the video tag.
                  </video>
                </div>
                {vid.title && <h3 className="font-semibold text-lg">{vid.title}</h3>}
              </div>
            ))
          ) : (
            <p className="text-gray-400 col-span-full">No videos published yet.</p>
          )}
        </div>
      </div>
    </div>
